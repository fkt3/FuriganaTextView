...
public class FuriganaView extends TextView {
    private class TextFurigana {
        ...
        public void offset_set(float value) {
            m_offset = value;
        }
        ...
        public void draw(Canvas canvas, float x, float y) {
            x -= m_width / 2.0f;
            m_paint_f.setColor(getCurrentTextColor());
    ...
    private class TextNormal {
        ...
        public float draw(Canvas canvas, float x, float y) {
            if (m_is_marked) {
                ...
                m_paint_k_norm.setColor(getCurrentTextColor());
    ...
    private class LineNormal {
        ...
        public void add(Vector<TextNormal> text) {
            m_text.addAll(text);
        }
    ...
    // Text functions
    public void text_set(String text, int mark_s, int mark_e) {
        // Text
        m_paint_k_norm = new TextPaint(getPaint());
        m_paint_k_mark = new TextPaint(getPaint());
        m_paint_k_mark.setFakeBoldText(true);

        m_paint_f = new TextPaint(getPaint());
        m_paint_f.setTextSize(getPaint().getTextSize() / 2.0f);
